...
(VarPrice <- uniroot(f,c(cpr/2,cpr*2),RiskFunc="pop.sd",Rf=rf,cap=cap_pct)$root)
#should be 120.8843
(SVarPrice <- uniroot(f,c(cpr/2,cpr*2),RiskFunc="svar_n",Rf=rf,cap=cap_pct)$root)
#should be 113.5862
(VAR5PctPrice <- uniroot(f,c(cpr/2,cpr*2),RiskFunc="VAR5pct",Rf=rf,cap=cap_pct)$root)
#should be 117.5173
(VAR10PctPrice <- uniroot(f,c(cpr/2,cpr*2),RiskFunc="VAR10pct",Rf=rf,cap=cap_pct)$root)
#should be 114.0436
(VAR20PctPrice <- uniroot(f,c(cpr/2,cpr*2),RiskFunc="VAR20pct",Rf=rf,cap=cap_pct)$root)
